...
import { clearCard } from '../redux/actions/card'
...
function Card() {
    ...
    const { items, totalCount, totalPrice } = useSelector(({ card }) => card)

    const cartArrayItems = Object.keys(items).map(key => items[key].items);

    const onClearCard = () => {
        if (window.confirm('Czy na pewno chcesz usunąć wszystko z koszyka?')) {
            dispatch(clearCard())
        }
    }
    return (
                    ...
                    Object.keys(items).length === 0 ?
                        ...
                        : <div className="cart">
                            <div className="cart__top">
                                ...
                                <div className="cart__clear" onClick={onClearCard}>
                            ...
                            <div className="content__items">
                                {
                                    cartArrayItems.map((pizza, index) =>
                                        <CardItem
                                            key={index}
                                            {...pizza[0]}
                                        />)
                            ...
                            <div className="cart__bottom">
                                <div className="cart__bottom-details">
                                    <span> Razem pizze: <b>{totalCount} sztuk</b> </span>
                                    <span> Cena zamówienia: <b>{totalPrice} zł</b> </span>
                                </div>
                                <div className="cart__bottom-buttons">
                                    ...
                                    <Button className='button pay-btn' onClick={() => alert('Dziękujemy za zakupy!')}>
                                        <span>Zapłać</span>
                                    </Button>
                                </div>
                            </div>
